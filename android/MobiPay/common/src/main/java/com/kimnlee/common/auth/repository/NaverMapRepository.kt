...
suspend fun getAddressFromCoords(latlng : LatLng): String { //latitude: Double, longitude: Double
    ...
    if (response?.status?.code == 0 && response.results.isNotEmpty()) {
        // 도로명 주소 찾기
        val addrCodeResult  = response.results.find { it.name == "addr" }
        if (addrCodeResult != null) {
            return buildString {
                append(addrCodeResult.region.area1.name) // 시/도
                append(" ")
                append(addrCodeResult.region.area2.name) // 구/군
                append(" ")
                append(addrCodeResult.region.area3.name) // 동
                append(" ")
                // land 정보 처리
                addrCodeResult.land?.let { land ->
